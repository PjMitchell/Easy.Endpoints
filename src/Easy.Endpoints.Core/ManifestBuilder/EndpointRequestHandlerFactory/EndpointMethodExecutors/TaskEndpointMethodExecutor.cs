...
namespace Easy.Endpoints
{
    ...
    internal class ValueTaskEndpointMethodExecutor : NoResultEndpointMethodExecutor
    {
        private readonly TaskEndpointHandleExcutor executor;

        public ValueTaskEndpointMethodExecutor(Type endpointType, MethodInfo handleMethod) : base(endpointType)
        {
            executor = BuildHandler(endpointType, handleMethod);
        }

        public override async Task Execute(object instance, object?[] parameters) => await executor(instance, parameters);
        ...
        public delegate ValueTask TaskEndpointHandleExcutor(object endpoint, object?[] context);
    }
}
